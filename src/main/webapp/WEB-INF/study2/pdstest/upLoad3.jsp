...
<head>
  ...
  <script>
    ...
    let fileBoxAppend = () => {
    	cnt++;
    	let fileBox = "";
    	fileBox += '<div class ="d-flex" id="fBox'+cnt+'">';
    	fileBox += '<input type="file" name="fName'+cnt+'" id="file'+cnt+'" class="form-control-file border mb-2 mr-2"/>';
    	fileBox += '<button type="button" onclick="deleteBox('+cnt+')" class="btn btn-warning form-control">삭제</button>';
    	fileBox += '</div>';
    	$("#file_box_demo").append(fileBox);
    }
    let deleteBox = (cnt) => {
    	$("#fBox"+cnt).remove();
    }
    function fCheck() {
    	...
    	const maxSize = 1024 * 1024 * 10;		// 업로드 가능한 최대파일의 용량은 10MByte로 한다.
